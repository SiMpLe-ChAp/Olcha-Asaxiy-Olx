    ...
    <div className='cart__wrapper container'>
      <div className='cart__wrapper-content '>
        {data?.map((item) => (
                  ...
                  <div className="small__cardd">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <img src={item.url} alt="" />&nbsp;&nbsp;&nbsp;&nbsp;
                    <div className="columnn">
                      <h1>{item.title}</h1>
                      <h2>STP-Group</h2>
                      <button className='intexx'  onClick={() => dispatch(removeFromCart(item.id))}>Удалить</button>
                    </div>
                    <div className="columnn__two">
                      <div className="roj">
              <button disabled={item.quantity < 1} onClick={() => dispatch(decrementCart(item))}>-</button>&nbsp;&nbsp;&nbsp;
              <span>{item.quantity}</span>&nbsp;&nbsp;&nbsp;&nbsp;
              <button onClick={() => dispatch(addToCart(item))}>+</button>
                      </div>&nbsp;&nbsp;&nbsp;&nbsp;
                      <h2 className='moro'>{item.price * item.quantity} so'm</h2>
                    </div>
                  </div>
                  <br /><br />
                </div>
